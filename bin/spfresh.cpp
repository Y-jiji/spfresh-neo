...
static float CalculateRecallSPFresh(SPTAG::VectorIndex* index, std::vector<SPTAG::QueryResult>& results, const std::vector<std::set<SPTAG::SizeType>>& truth, int K, int truthK, std::shared_ptr<SPTAG::VectorSet> querySet, std::shared_ptr<SPTAG::VectorSet> vectorSet, SPTAG::SizeType NumQuerys, std::ofstream* log = nullptr, bool debug = false) {
    ...
    for (SPTAG::SizeType i = 0; i < NumQuerys; i++) {
        ...
        for (SPTAG::SizeType id : truth[i]) {
            for (int j = 0; j < K; j++) {
                if (visited[j] || results[i].GetResult(j)->VID < 0)
                    continue;
                if (vectorSet != nullptr) {
    ...
    LOG(Helper::LogLevel::LL_Info, "\nRecall Distribution:\n");
    PrintPercentiles<float, float>(thisrecall, [](const float recall) -> float { return recall; }, "%.3lf", true);
...
template <typename ValueType>
double SearchSequential(SPANN::Index<ValueType>* p_index, int p_numThreads, std::vector<SPTAG::QueryResult>& p_results, std::vector<SPANN::SearchStats>& p_stats, int p_maxQueryCount, int p_internalResultNum) {
...
void PrintStats(std::vector<SPANN::SearchStats>& stats) {
    LOG(Helper::LogLevel::LL_Info, "\nEx Elements Count:\n");
    PrintPercentiles<double, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> double { return ss.m_totalListElementsCount; }, "%.3lf");

    LOG(Helper::LogLevel::LL_Info, "\nHead Latency Distribution:\n");
    PrintPercentiles<double, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> double { return ss.m_totalSearchLatency - ss.m_exLatency; }, "%.3lf");

    LOG(Helper::LogLevel::LL_Info, "\nSetup Latency Distribution:\n");
    PrintPercentiles<double, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> double { return ss.m_exSetUpLatency; }, "%.3lf");

    LOG(Helper::LogLevel::LL_Info, "\nComp Latency Distribution:\n");
    PrintPercentiles<double, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> double { return ss.m_compLatency; }, "%.3lf");

    LOG(Helper::LogLevel::LL_Info, "\nSPDK Latency Distribution:\n");
    PrintPercentiles<double, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> double { return ss.m_diskReadLatency; }, "%.3lf");

    LOG(Helper::LogLevel::LL_Info, "\nEx Latency Distribution:\n");
    PrintPercentiles<double, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> double { return ss.m_exLatency; }, "%.3lf");

    LOG(Helper::LogLevel::LL_Info, "\nTotal Latency Distribution:\n");
    PrintPercentiles<double, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> double { return ss.m_totalSearchLatency; }, "%.3lf");

    LOG(Helper::LogLevel::LL_Info, "\nTotal Disk Page Access Distribution(KB):\n");
    PrintPercentiles<int, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> int { return ss.m_diskAccessCount; }, "%4d");

    LOG(Helper::LogLevel::LL_Info, "\nTotal Disk IO Distribution:\n");
    PrintPercentiles<int, SPANN::SearchStats>(stats, [](const SPANN::SearchStats& ss) -> int { return ss.m_diskIOCount; }, "%4d");
...
template <typename ValueType>
void StableSearch(SPANN::Index<ValueType>* p_index, int numThreads, std::shared_ptr<SPTAG::VectorSet> querySet, std::shared_ptr<SPTAG::VectorSet> vectorSet, int avgStatsNum, int queryCountLimit, int internalResultNum, std::string& truthFileName, SPANN::Options& p_opts, double second = 0, bool showStatus = true) {
    if (avgStatsNum == 0)
        return;
...
template <typename ValueType>
void InsertVectorsBySet(SPANN::Index<ValueType>* p_index, int insertThreads, std::shared_ptr<SPTAG::VectorSet> vectorSet, std::vector<SPTAG::SizeType>& insertSet, std::vector<SPTAG::SizeType>& mapping, int updateSize, SPANN::Options& p_opts) {
    StopWSPFresh sw;
    std::vector<std::thread> threads;
        ...
        while (true) {
            ...
            if (index < updateSize) {
                ...
                if (p_opts.m_stressTest)
                    p_index->DeleteIndex(mapping[insertSet[index]]);
    ...
    PrintPercentiles<double, double>(latency_vector, [](const double& ss) -> double { return ss; }, "%.3lf");
}

template <typename ValueType>
void DeleteVectorsBySet(SPANN::Index<ValueType>* p_index, int deleteThreads, std::shared_ptr<SPTAG::VectorSet> vectorSet, std::vector<SPTAG::SizeType>& deleteSet, std::vector<SPTAG::SizeType>& mapping, int updateSize, SPANN::Options& p_opts, int batch) {
    int avgQPS = p_opts.m_deleteQPS / deleteThreads;
    if (p_opts.m_deleteQPS == -1)
        avgQPS = -1;
    std::vector<double> latency_vector(updateSize);
    std::vector<std::thread> threads;
    StopWSPFresh sw;
...
void SteadyStateSPFresh(SPANN::Index<ValueType>* p_index) {
    ...
    if (p_opts.m_endVectorNum == -1)
        p_opts.m_endVectorNum = curCount;
    ...
    for (int i = 0; i < days; i++) {
        ...
        if (!p_opts.m_stressTest)
            ...
            LoadUpdateTraceStressTest(traceFileName, updateSize, insertSet);
        if (!p_opts.m_loadAllVectors) {
        ...
        if (!p_opts.m_stressTest) {
            delete_future =
                std::async(std::launch::async, DeleteVectorsBySet<ValueType>, p_index, 1, vectorSet, std::ref(deleteSet), std::ref(mapping), updateSize, std::ref(p_opts), i);
        }

        std::future_status delete_status;

        std::future<void> insert_future =
            std::async(std::launch::async, InsertVectorsBySet<ValueType>, p_index, insertThreads, vectorSet, std::ref(insertSet), std::ref(mapping), updateSize, std::ref(p_opts));
        ...
        do {
            ...
            if (insert_status == std::future_status::timeout || delete_status == std::future_status::timeout) {
                ...
                p_index->GetDBStat();
                if (p_opts.m_searchDuringUpdate)
                    StableSearch(p_index, numThreads, querySet, vectorSet, searchTimes, p_opts.m_queryCountLimit, internalResultNum, tempFileName, p_opts, sw.getElapsedSec(), showStatus);
                p_index->GetDBStat();
            }
        ...
        if (!p_opts.m_stressTest)
            truthFileName = p_opts.m_truthFilePrefix + std::to_string(i);
        else
            truthFileName = p_opts.m_truthPath;

        p_opts.m_calTruth = calTruthOrigin;
        if (p_opts.m_onlySearchFinalBatch && days - 1 != i)
            continue;
        p_index->StopMerge();
        if (p_opts.m_maxInternalResultNum != -1) {
...
template <typename ValueType>
void InsertVectors(SPANN::Index<ValueType>* p_index, int insertThreads, std::shared_ptr<SPTAG::VectorSet> vectorSet, int curCount, int step, SPANN::Options& p_opts) {
    StopWSPFresh sw;
    std::vector<std::thread> threads;
...
void UpdateSPFresh(SPANN::Index<ValueType>* p_index) {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        std::future<void> insert_future =
            std::async(std::launch::async, InsertVectors<ValueType>, p_index, insertThreads, vectorSet, curCount, step, std::ref(p_opts));
        ...
        do {
            ...
            if (insert_status == std::future_status::timeout) {
                ...
                p_index->GetDBStat();
                if (p_opts.m_searchDuringUpdate)
                    StableSearch(p_index, numThreads, querySet, vectorSet, searchTimes, p_opts.m_queryCountLimit, internalResultNum, tempFileName, p_opts, sw.getElapsedSec());
            }
        ...
        p_opts.m_calTruth = calTruthOrigin;
        if (p_opts.m_onlySearchFinalBatch && batch - 1 != i)
            continue;
        // p_index->ForceGC();
        // p_index->ForceCompaction();
        p_index->StopMerge();
