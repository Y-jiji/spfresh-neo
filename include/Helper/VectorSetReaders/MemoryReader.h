...
class MemoryVectorReader : public VectorSetReader {
   public:
    MemoryVectorReader(std::shared_ptr<ReaderOptions> p_options, std::shared_ptr<VectorSet> p_vectors) : VectorSetReader(p_options), m_vectors(p_vectors) {}
    ...
    virtual std::shared_ptr<VectorSet> GetVectorSet(SizeType start = 0, SizeType end = -1) const {
        if (end < 0 || end > m_vectors->Count())
            end = m_vectors->Count();
        return std::shared_ptr<VectorSet>(new BasicVectorSet(ByteArray((std::uint8_t*)(m_vectors->GetVector(start)), (end - start) * m_vectors->PerVectorDataSize(), false), m_vectors->GetValueType(), m_vectors->Dimension(), end - start));
    }
