...
namespace SPTAG::Helper::LockFree {
template <typename T>
class LockFreeVector {
   ...
   public:
    ...
    inline const T& back() const {
        auto idx = m_size - 1;
        return *(m_blocks[idx >> m_blockSizeEx] + (idx & m_blockSize));
    }
    inline void clear() {
        m_size = 0;
    }
    ...
    inline const T& operator[](std::uint64_t offset) const {
        return *(m_blocks[offset >> m_blockSizeEx] + (offset & m_blockSize));
    }
    ...
    bool save(std::shared_ptr<Helper::DiskIO> out) {
        ...
        for (int i = 0; i < blockNum; i++)
            if (out->WriteBinary(sizeof(T) * (m_blockSize + 1), (char*)(m_blocks[i])) != sizeof(T) * (m_blockSize + 1))
                return false;

        auto remainNum = (m_size & m_blockSize);
        if (remainNum > 0 && out->WriteBinary(sizeof(T) * remainNum, (char*)(m_blocks[blockNum])) != sizeof(T) * remainNum)
            return false;
        return true;
    }

    bool load(std::shared_ptr<Helper::DiskIO> in, size_t length) {
        if (m_size + length > (m_blockSize + 1) * m_blocks.capacity())
            return false;
        ...
        while (written < length) {
            ...
            auto curBlockPos = ((m_size + written) & m_blockSize);
            auto toWrite = min(m_blockSize + 1 - curBlockPos, length - written);
            if (in->ReadBinary(sizeof(T) * toWrite, (char*)(m_blocks[currBlock] + curBlockPos)) != sizeof(T) * toWrite)
                return false;
            written += toWrite;
        }
        m_size += written;
