   ...
   public:
    ...
    void Initialize(SizeType rows_, DimensionType cols_, SizeType rowsInBlock_, SizeType capacity_, const void* data_ = nullptr, bool shareOwnership_ = true, std::shared_ptr<std::vector<char*>> incBlocks_ = nullptr, int colStart_ = 0, int rowEnd_ = -1) {
        ...
        if (data_ == nullptr || !shareOwnership_) {
            ...
            if (data_ != nullptr)
                memcpy(data, data_, ((size_t)rows) * cols);
            else
        ...
        if (incBlocks == nullptr)
            incBlocks.reset(new std::vector<char*>());
        incBlocks->reserve((static_cast<std::int64_t>(capacity_) + rowsInBlock) >> rowsInBlockEx);
    ...
    inline std::uint64_t BufferSize() const {
        return sizeof(SizeType) + sizeof(DimensionType) + sizeof(T) * R() * C();
    }
...
#undef GETITEM
    ...
    ErrorCode Save(std::string sDataPointsFileName) const {
        LOG(Helper::LogLevel::LL_Info, "Save %s To %s\n", name.c_str(), sDataPointsFileName.c_str());
        auto ptr = f_createIO();
        if (ptr == nullptr || !ptr->Initialize(sDataPointsFileName.c_str(), std::ios::binary | std::ios::out))
    ...
    ErrorCode Load(std::shared_ptr<Helper::DiskIO> pInput, SizeType blockSize, SizeType capacity) {
        ...
        if (data == nullptr)
            Initialize(rows, mycols, blockSize, capacity);
    ...
    ErrorCode Load(std::string sDataPointsFileName, SizeType blockSize, SizeType capacity) {
        LOG(Helper::LogLevel::LL_Info, "Load %s From %s\n", name.c_str(), sDataPointsFileName.c_str());
        auto ptr = f_createIO();
        if (ptr == nullptr || !ptr->Initialize(sDataPointsFileName.c_str(), std::ios::binary | std::ios::in))
    ...
    ErrorCode Refine(const std::vector<SizeType>& indices, COMMON::Dataset<T>& dataset) const {
        ...
        if (dataset.data == nullptr)
            dataset.Initialize(newrows, mycols, rowsInBlock + 1, static_cast<SizeType>(incBlocks->capacity() * (rowsInBlock + 1)));
...
template <typename T>
ErrorCode LoadOptDatasets(std::shared_ptr<Helper::DiskIO> pVectorsInput, std::shared_ptr<Helper::DiskIO> pGraphInput, Dataset<T>& pVectors, Dataset<SizeType>& pGraph, DimensionType pNeighborhoodSize, SizeType blockSize, SizeType capacity) {
    SizeType VR, GR;
    DimensionType VC, GC;
