...
class RelativeNeighborhoodGraph {
   public:
    RelativeNeighborhoodGraph() : m_iTPTNumber(32),
                                  m_iTPTLeafSize(2000),
                                  m_iSamples(1000),
                                  ...
                                  m_rebuild(0) {
        m_pNeighborhoodGraph.SetName("RNG");
    }
    ...
    void InsertNeighbors(VectorIndex* index, const SizeType node, SizeType insertNode, float insertDist) {
        ...
        for (DimensionType i = 0; i < m_iNeighborhoodSize; i++) {
            ...
            if (futureNode < 0 || futureNode >= index->GetNumSamples())
                break;
    ...
    void RebuildNeighbors(VectorIndex* index, const SizeType node, SizeType* nodes, const BasicResult* queryResults, const int numResults) {
        ...
        for (DimensionType j = count; j < m_iNeighborhoodSize; j++)
            nodes[j] = -1;
    }
...
#pragma omp parallel for schedule(dynamic)
        for (SizeType i = 0; i < m_iGraphSize; i++) {
            ...
            int total = 0;
            for (DimensionType j = rebuildstart; j < m_iNeighborhoodSize * 2; j++)
                if (outnodes[j] >= 0 && indegree[outnodes[j]] < rebuild_threshold) {
                    reserve[j] = true;
                    total++;
                }

            for (DimensionType j = rebuildstart; j < m_iNeighborhoodSize * 2 && total < m_iNeighborhoodSize - rebuildstart; j++) {
                if (!reserve[j]) {
                    reserve[j] = true;
                    total++;
                }
            }
            for (DimensionType j = rebuildstart, z = rebuildstart; j < m_iNeighborhoodSize; j++) {
                while (!reserve[z])
                    z++;
                if (outnodes[j] >= 0)
...
#pragma omp parallel for schedule(dynamic)
        for (SizeType i = 0; i < R; i++) {
            if ((i * 5) % R == 0)
                LOG(Helper::LogLevel::LL_Info, "Refine %d%%\n", static_cast<int>(i * 1.0 / R * 100));
    ...
    ErrorCode LoadGraph(std::shared_ptr<Helper::DiskIO> input, SizeType blockSize, SizeType capacity) {
        ...
        if ((ret = m_pNeighborhoodGraph.Load(input, blockSize, capacity)) != ErrorCode::Success)
            return ret;
    ...
    ErrorCode LoadGraph(std::string sGraphFilename, SizeType blockSize, SizeType capacity) {
        ErrorCode ret = ErrorCode::Success;
        if ((ret = m_pNeighborhoodGraph.Load(sGraphFilename, blockSize, capacity)) != ErrorCode::Success)
            return ret;
    ...
    ErrorCode LoadGraph(char* pGraphMemFile, SizeType blockSize, SizeType capacity) {
        ErrorCode ret = ErrorCode::Success;
        if ((ret = m_pNeighborhoodGraph.Load(pGraphMemFile, blockSize, capacity)) != ErrorCode::Success)
            return ret;
    ...
    ErrorCode SaveGraph(std::string sGraphFilename) const {
        ...
        auto ptr = f_createIO();
        if (ptr == nullptr || !ptr->Initialize(sGraphFilename.c_str(), std::ios::binary | std::ios::out))
    ...
    inline SizeType* operator[](SizeType index) {
        return m_pNeighborhoodGraph[index];
    }

    inline const SizeType* operator[](SizeType index) const {
        return m_pNeighborhoodGraph[index];
    }
    ...
    inline std::string Type() const {
        return m_pNeighborhoodGraph.Name();
    }
