...
template <typename T, typename R>
inline float KmeansAssign(const Dataset<T>& data, std::vector<SizeType>& indices, const SizeType first, const SizeType last, KmeansArgs<T>& args, const bool updateCenters, float lambda) {
    float currDist = 0;
    SizeType totalSize = last - first;

    // TODO - compile-time options for MAX_DIM and metric
    computeKmeansGPU<T, float, 100>(data, indices, first, last, args._K, args._D, args._DK, lambda, args.centers, args.label, args.counts, args.newCounts, args.newCenters, args.clusterIdx, args.clusterDist, args.weightedCounts, args.newWeightedCounts, 0, updateCenters);
}
...
inline float KmeansAssign(const Dataset<T>& data, std::vector<SizeType>& indices, const SizeType first, const SizeType last, KmeansArgs<T>& args, const bool updateCenters, float lambda) {
    ...
    if (updateCenters) {
        for (int i = 1; i < args._T; i++) {
            ...
            for (size_t j = 0; j < ((size_t)args._DK) * args._RD; j++)
...
template <typename T, typename R>
inline float InitCenters(const Dataset<T>& data, std::vector<SizeType>& indices, const SizeType first, const SizeType last, KmeansArgs<T>& args, int samples, int tryIters) {
...
template <typename T, typename R>
float TryClustering(const Dataset<T>& data, std::vector<SizeType>& indices, const SizeType first, const SizeType last, KmeansArgs<T>& args, int samples = 1000, float lambdaFactor = 100.0f, bool debug = false, IAbortOperation* abort = nullptr, bool virtualCenter = false) {
    ...
    for (int iter = 0; iter < 100; iter++) {
        ...
        */
        ...
        if (abort && abort->ShouldAbort())
            return 0;
        if (currDiff < 1e-3 || noImprovement >= 5)
            break;
    }
    ...
    if (!virtualCenter) {
        ...
        for (int k = 0; k < args._DK; k++) {
            if (args.clusterIdx[k] != -1)
                std::memcpy(args.centers + k * args._D, data[args.clusterIdx[k]], sizeof(T) * args._D);
        }
    }
    ...
    for (int i = 0; i < args._DK; i++) {
        if (args.counts[i] > maxCount)
            maxCount = args.counts[i];
        if (args.counts[i] < minCount)
            minCount = args.counts[i];
...
class BKTree {
   public:
    ...
    inline BKTNode& operator[](SizeType index) {
        return m_pTreeRoots[index];
    }

    inline SizeType size() const {
        return (SizeType)m_pTreeRoots.size();
    }
    ...
    inline const std::unordered_map<SizeType, SizeType>& GetSampleMap() const {
    ...
    void BuildTrees(const Dataset<T>& data, DistCalcMethod distMethod, int numOfThreads, std::vector<SizeType>* indices = nullptr, std::vector<SizeType>* reverseIndices = nullptr, bool dynamicK = false, IAbortOperation* abort = nullptr) {
        ...
        if (indices == nullptr) {
            ...
            for (SizeType i = 0; i < localindices.size(); i++)
                localindices[i] = i;
        } else {
    ...
    ErrorCode SaveTrees(std::string sTreeFileName) const {
        ...
        auto ptr = f_createIO();
        if (ptr == nullptr || !ptr->Initialize(sTreeFileName.c_str(), std::ios::binary | std::ios::out))
    ...
    ErrorCode LoadTrees(char* pBKTMemFile) {
        ...
        if (m_pTreeRoots.size() > 0 && m_pTreeRoots.back().centerid != -1)
            m_pTreeRoots.emplace_back(-1);
    ...
    ErrorCode LoadTrees(std::string sTreeFileName) {
        ...
        auto ptr = f_createIO();
        if (ptr == nullptr || !ptr->Initialize(sTreeFileName.c_str(), std::ios::binary | std::ios::in))
    ...
    void InitSearchTrees(const Dataset<T>& data, std::function<float(const T*, const T*, DimensionType)> fComputeDistance, COMMON::QueryResultSet<T>& p_query, COMMON::WorkSpace& p_space) const {
        for (char i = 0; i < m_iTreeNumber; i++) {
            ...
            if (node.childStart < 0) {
                ...
                float FactorQ = 1.1f;
                int MaxBFSNodes = 100;
                p_space.m_currBSPTQueue.Resize(MaxBFSNodes);
                p_space.m_nextBSPTQueue.Resize(MaxBFSNodes);
    ...
    template <typename T>
    void SearchTrees(const Dataset<T>& data, std::function<float(const T*, const T*, DimensionType)> fComputeDistance, COMMON::QueryResultSet<T>& p_query, COMMON::WorkSpace& p_space, const int p_limits) const {
        while (!p_space.m_SPTQueue.empty()) {
            ...
            if (tnode.childStart < 0) {
                ...
                if (p_space.m_iNumberOfCheckedLeaves >= p_limits)
                    break;
            } else {
                if (!p_space.CheckAndSet(tnode.centerid)) {
                    p_space.m_NGQueue.insert(NodeDistPair(tnode.centerid, bcell.distance));
