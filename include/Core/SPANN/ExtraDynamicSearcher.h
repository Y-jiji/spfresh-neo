   ...
   public:
    ...
    void CalculatePostingDistribution(SPTAG::BKT::Index<ValueType>* p_index) {
        ...
        for (int i = 0; i < page + 2; ++i) {
            if (i <= 2) {
                if (i == 0)
                    LOG(Helper::LogLevel::LL_Info, "0 ~ 512 B: %d, \n", sizeDistribution[0] - deletedHead);
                else if (i == 1)
                    LOG(Helper::LogLevel::LL_Info, "512 B ~ 1 KB: %d, \n", sizeDistribution[1]);
                else
                    LOG(Helper::LogLevel::LL_Info, "1 KB ~ 4 KB: %d, \n", sizeDistribution[2]);
            } else
    ...
    bool RNGSelection(std::vector<Edge>& selections, ValueType* queryVector, SPTAG::BKT::Index<ValueType>* p_index, SizeType p_fullID, int& replicaCount, int checkHeadID = -1) {
        ...
        for (int i = 0; i < queryResults.GetResultNum() && replicaCount < m_opt->m_replicaCount; ++i) {
            ...
            bool rngAccpeted = true;
            for (int j = 0; j < replicaCount; ++j) {
                float nnDist = p_index->ComputeDistance(p_index->GetSample(queryResult->VID), p_index->GetSample(selections[j].node));
