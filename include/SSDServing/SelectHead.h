...
namespace SPTAG::SSDServing::SelectHead {
struct BKTNodeInfo {
...
void CalcLeafSize(int p_nodeID, const std::shared_ptr<COMMON::BKTree> p_tree, std::unordered_map<int, int>& p_counter) {
...
void DfsAnalyze(int p_nodeID, const std::shared_ptr<COMMON::BKTree> p_tree, std::shared_ptr<VectorSet> p_vectorSet, const SPANN::Options& p_opts, int p_height, std::vector<BKTNodeInfo>& p_nodeInfos) {
...
void DfsSelect(int p_nodeID, const std::shared_ptr<SPTAG::COMMON::BKTree> p_tree, std::vector<HeadCandidate>& p_candidates, std::vector<BKTNodeInfo>& p_nodeInfos) {
    const auto& node = (*p_tree)[p_nodeID];
...
// Return height of current Node.
int DfsCovered(int p_nodeID, const std::shared_ptr<SPTAG::COMMON::BKTree> p_tree, const std::unordered_set<int>& p_candidates, bool p_covered) {
    const auto& node = (*p_tree)[p_nodeID];
...
int SelectHeadDynamicallyInternal(const std::shared_ptr<COMMON::BKTree> p_tree, int p_nodeID, const SPANN::Options& p_opts, std::vector<int>& p_selected) {
...
void SelectHeadDynamically_Old(const std::shared_ptr<COMMON::BKTree> p_tree, int p_vectorCount, const SPANN::Options& p_opts, std::vector<int>& p_selected) {
    p_selected.clear();
...
void SelectHeadDynamically(const std::shared_ptr<COMMON::BKTree> p_tree, int p_vectorCount, const SPANN::Options& p_opts, std::vector<int>& p_selected) {
    std::mt19937 random;
    p_selected.clear();
    p_selected.reserve(p_vectorCount);
...
void AdjustOptions(SPANN::Options& p_opts, int p_vectorCount) {
    if (p_opts.m_headVectorCount != 0)
        p_opts.m_ratio = p_opts.m_headVectorCount * 1.0 / p_vectorCount;
...
ErrorCode Bootstrap(SPANN::Options& opts) {
    ...
    if (opts.m_distCalcMethod == DistCalcMethod::Cosine)
        vectorSet->Normalize(opts.m_iSelectHeadNumberOfThreads);
