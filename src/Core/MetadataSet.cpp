...
SPTAG::ErrorCode
SPTAG::MetadataSet::RefineMetadata(std::vector<SPTAG::SizeType>& indices, std::shared_ptr<SPTAG::MetadataSet>& p_newMetadata, std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize) const {
...
void SPTAG::MetadataSet::AddBatch(SPTAG::MetadataSet& data) {
...
bool SPTAG::FileMetadataSet::Available() const {
    std::shared_lock<std::shared_timed_mutex> lock(*static_cast<std::shared_timed_mutex*>(m_lock.get()));
...
SPTAG::FileMetadataSet::BufferSize() const {
    std::shared_lock<std::shared_timed_mutex> lock(*static_cast<std::shared_timed_mutex*>(m_lock.get()));
    return std::make_pair(m_offsets.back(), sizeof(SPTAG::SizeType) + sizeof(std::uint64_t) * m_offsets.size());
}

void SPTAG::FileMetadataSet::Add(const SPTAG::ByteArray& data) {
    std::unique_lock<std::shared_timed_mutex> lock(*static_cast<std::shared_timed_mutex*>(m_lock.get()));
...
SPTAG::FileMetadataSet::SaveMetadata(const std::string& p_metaFile, const std::string& p_metaindexFile) {
    ...
    }
    {
        ...
        std::rename((p_metaFile + "_tmp").c_str(), p_metaFile.c_str());
        std::rename((p_metaindexFile + "_tmp").c_str(), p_metaindexFile.c_str());
        if (!m_fp->Initialize(p_metaFile.c_str(), std::ios::binary | std::ios::in))
...
SPTAG::ErrorCode
SPTAG::MemMetadataSet::Init(std::shared_ptr<SPTAG::Helper::DiskIO> p_metain, std::shared_ptr<SPTAG::Helper::DiskIO> p_metaindexin, std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize) {
...
SPTAG::MemMetadataSet::MemMetadataSet(std::shared_ptr<SPTAG::Helper::DiskIO> p_metain, std::shared_ptr<SPTAG::Helper::DiskIO> p_metaindexin, std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize) {
...
SPTAG::MemMetadataSet::MemMetadataSet(const std::string& p_metafile, const std::string& p_metaindexfile, std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize) {
    std::shared_ptr<SPTAG::Helper::DiskIO> ptrMeta = SPTAG::f_createIO(), ptrMetaIndex = SPTAG::f_createIO();
...
SPTAG::MemMetadataSet::MemMetadataSet(SPTAG::ByteArray p_metadata, SPTAG::ByteArray p_offsets, SPTAG::SizeType p_count, std::uint64_t p_blockSize, std::uint64_t p_capacity, std::uint64_t p_metaSize)
    : m_count(p_count), m_metadataHolder(std::move(p_metadata)) {
    m_pOffsets.reset(new MetadataOffsets, std::default_delete<MetadataOffsets>());
    auto& m_offsets = *static_cast<MetadataOffsets*>(m_pOffsets.get());
    m_offsets.reserve(p_blockSize, p_capacity);
...
bool SPTAG::MemMetadataSet::Available() const {
    auto& m_offsets = *static_cast<MetadataOffsets*>(m_pOffsets.get());
...
SPTAG::MemMetadataSet::BufferSize() const {
    ...
    return std::make_pair(m_offsets[n - 1], sizeof(SPTAG::SizeType) + sizeof(std::uint64_t) * n);
}

void SPTAG::MemMetadataSet::Add(const SPTAG::ByteArray& data) {
    auto& m_offsets = *static_cast<MetadataOffsets*>(m_pOffsets.get());
    std::unique_lock<std::shared_timed_mutex> lock(*static_cast<std::shared_timed_mutex*>(m_lock.get()));
    m_newdata.insert(m_newdata.end(), data.Data(), data.Data() + data.Length());
