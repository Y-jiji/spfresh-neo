...
void 
SPTAG::VectorIndex::SetMetadata(SPTAG::MetadataSet* p_new) {
...
void
SPTAG::VectorIndex::UpdateMetaMapping(const std::string& meta, SPTAG::SizeType i)
{
...
void
SPTAG::VectorIndex::BuildMetaMapping(bool p_checkDeleted)
{
    m_metadataManager.BuildMetaMapping(m_pMetadata.get(), GetNumSamples(),
        std::function<bool(SPTAG::SizeType)>([this](SPTAG::SizeType idx) -> bool {
...
{
    if (!m_bReady || GetNumSamples() - GetNumDeleted() == 0) return SPTAG::ErrorCode::EmptyIndex;
    ...
    SPTAG::ErrorCode ret = SPTAG::ErrorCode::Success;
    {
        ...
        if (p_configStream == nullptr || !p_configStream->Initialize(nullptr, std::ios::out)) return SPTAG::ErrorCode::EmptyDiskIO;
        if ((ret = SaveIndexConfig(p_configStream)) != SPTAG::ErrorCode::Success) return ret;
    ...
    for (size_t i = 0; i < p_indexBlobs.size(); i++)
    {
        ...
        if (ptr == nullptr || !ptr->Initialize((char*)p_indexBlobs[i].Data(), std::ios::binary | std::ios::out, p_indexBlobs[i].Length())) return SPTAG::ErrorCode::EmptyDiskIO;
    ...
    else
    {
        ...
        if (SPTAG::ErrorCode::Success == ret) ret = SaveIndexData(p_indexStreams);
    }
...
SPTAG::VectorIndex::SaveIndex(const std::string& p_folderPath)
{
    ...
    SPTAG::ErrorCode ret = SPTAG::ErrorCode::Success;
    {
        ...
        if (configFile == nullptr || !configFile->Initialize((folderPath + "indexloader.ini").c_str(), std::ios::out)) return SPTAG::ErrorCode::FailedCreateFile;
        if ((ret = SaveIndexConfig(configFile)) != SPTAG::ErrorCode::Success) return ret;
    }
    ...
    for (std::string& f : *indexfiles) {
        ...
        if (ptr == nullptr || !ptr->Initialize(newfile.c_str(), std::ios::binary | std::ios::out)) return SPTAG::ErrorCode::FailedCreateFile;
    ...
    else
    {
        ...
        if (SPTAG::ErrorCode::Success == ret) ret = SaveIndexData(handles);
    }
    if (m_pMetadata != nullptr) metaStart += 2;
...
SPTAG::VectorIndex::SaveIndexToFile(const std::string& p_file, SPTAG::IAbortOperation* p_abort)
{
    ...
    if (fp == nullptr || !fp->Initialize(p_file.c_str(), std::ios::binary | std::ios::out)) return SPTAG::ErrorCode::FailedCreateFile;

    auto mp = std::shared_ptr<SPTAG::Helper::DiskIO>(new SPTAG::Helper::SimpleBufferIO());
    if (mp == nullptr || !mp->Initialize(nullptr, std::ios::binary | std::ios::out)) return SPTAG::ErrorCode::FailedCreateFile;
    SPTAG::ErrorCode ret = SPTAG::ErrorCode::Success;
    if ((ret = SaveIndexConfig(mp)) != SPTAG::ErrorCode::Success) return ret;

    std::uint64_t configSize = mp->TellP();
    mp->ShutDown();

    IOBINARY(fp, WriteBinary, sizeof(configSize), (char*)&configSize);
    if ((ret = SaveIndexConfig(fp)) != SPTAG::ErrorCode::Success) return ret;

    if (p_abort != nullptr && p_abort->ShouldAbort()) ret = SPTAG::ErrorCode::ExternalAbort;
    else {
        ...
        else
        {
            ...
            if (SPTAG::ErrorCode::Success == ret && m_pMetadata != nullptr) ret = m_pMetadata->SaveMetadata(fp, fp);
        }
        if (SPTAG::ErrorCode::Success == ret && m_pQuantizer) {
    ...
    if (ret != SPTAG::ErrorCode::Success) std::remove(p_file.c_str());
    return ret;
}


SPTAG::ErrorCode
SPTAG::VectorIndex::BuildIndex(std::shared_ptr<SPTAG::VectorSet> p_vectorSet,
    std::shared_ptr<SPTAG::MetadataSet> p_metadataSet, bool p_withMetaIndex, bool p_normalized, bool p_shareOwnership)
{
...
SPTAG::VectorIndex::DeleteIndex(SPTAG::ByteArray p_meta) {
    if (!m_metadataManager.HasMetaMapping()) return SPTAG::ErrorCode::VectorNotFound;
...
#pragma omp parallel for num_threads(p_threadnum) schedule(dynamic,128)
        ...
        {
            if (ret == SPTAG::ErrorCode::ExternalAbort) continue;
...
SPTAG::VectorIndex::LoadIndex(const std::string& p_loaderFilePath, std::shared_ptr<SPTAG::VectorIndex>& p_vectorIndex)
{
    ...
    {
        auto fp = SPTAG::f_createIO();
        if (fp == nullptr || !fp->Initialize((folderPath + "indexloader.ini").c_str(), std::ios::in)) return SPTAG::ErrorCode::FailedOpenFile;
        if (SPTAG::ErrorCode::Success != iniReader.LoadIni(fp)) return SPTAG::ErrorCode::FailedParseValue;
    }
...
#undef DefineVectorValueType
    ...
    if (p_vectorIndex == nullptr) return SPTAG::ErrorCode::FailedParseValue;

    SPTAG::ErrorCode ret = SPTAG::ErrorCode::Success;
    if ((ret = p_vectorIndex->LoadIndexConfig(iniReader)) != SPTAG::ErrorCode::Success) return ret;
    ...
    if ((ret = p_vectorIndex->LoadIndexData(handles)) != SPTAG::ErrorCode::Success) return ret;

    size_t metaStart = p_vectorIndex->GetIndexFiles()->size();
    if (iniReader.DoesSectionExist("MetaData"))
    {
        p_vectorIndex->SetMetadata(new SPTAG::MemMetadataSet(handles[metaStart], handles[metaStart + 1],
    ...
    if (iniReader.DoesSectionExist("Quantizer")) {
        ...
        if (!p_vectorIndex->m_pQuantizer) return SPTAG::ErrorCode::FailedParseValue;
    }
...
{
    auto fp = SPTAG::f_createIO();
    if (fp == nullptr || !fp->Initialize(p_file.c_str(), std::ios::binary | std::ios::in)) return SPTAG::ErrorCode::FailedOpenFile;
    ...
    SPTAG::Helper::IniReader iniReader;
    {
        ...
        if (bufferhandle == nullptr || !bufferhandle->Initialize(config.data(), std::ios::in, configSize)) return SPTAG::ErrorCode::EmptyDiskIO;
        if (SPTAG::ErrorCode::Success != iniReader.LoadIni(bufferhandle)) return SPTAG::ErrorCode::FailedParseValue;
    }
...
#undef DefineVectorValueType
    ...
    std::vector<std::shared_ptr<SPTAG::Helper::DiskIO>> p_indexStreams(blobs, fp);
    if ((ret = p_vectorIndex->LoadIndexData(p_indexStreams)) != SPTAG::ErrorCode::Success) return ret;
...
SPTAG::VectorIndex::LoadIndex(const std::string& p_config, const std::vector<SPTAG::ByteArray>& p_indexBlobs, std::shared_ptr<SPTAG::VectorIndex>& p_vectorIndex)
{
    ...
    if (fp == nullptr || !fp->Initialize(p_config.c_str(), std::ios::in, p_config.size())) return SPTAG::ErrorCode::EmptyDiskIO;
    if (SPTAG::ErrorCode::Success != iniReader.LoadIni(fp)) return SPTAG::ErrorCode::FailedParseValue;
...
#undef DefineVectorValueType
    ...
    if ((p_vectorIndex->LoadIndexConfig(iniReader)) != SPTAG::ErrorCode::Success) return ret;

    if ((ret = p_vectorIndex->LoadIndexDataFromMemory(p_indexBlobs)) != SPTAG::ErrorCode::Success) return ret;
    ...
    if (iniReader.DoesSectionExist("MetaData") && p_indexBlobs.size() >= metaStart + 2)
    {
        ...
        p_vectorIndex->SetMetadata(new SPTAG::MemMetadataSet(p_indexBlobs[metaStart],
            SPTAG::ByteArray(pMetaIndex.Data() + sizeof(SPTAG::SizeType), pMetaIndex.Length() - sizeof(SPTAG::SizeType), false),
            *((SPTAG::SizeType*)pMetaIndex.Data()),
            p_vectorIndex->m_iDataBlockSize, p_vectorIndex->m_iDataCapacity, p_vectorIndex->m_iMetaRecordSize));
