...
void SPTAG::MetaDataManager::SetIndexName(const std::string& p_name) {
    m_sIndexName = p_name;
}
...
void SPTAG::MetaDataManager::SetMetadataFile(const std::string& p_file) {
...
void SPTAG::MetaDataManager::UpdateMetaMapping(const std::string& meta, SPTAG::SizeType i) {
    MetadataMap* ptr = static_cast<MetadataMap*>(m_pMetaToVec.get());
    auto iter = ptr->find(meta);
    if (iter != ptr->end()) {
    }
...
void SPTAG::MetaDataManager::BuildMetaMapping(SPTAG::MetadataSet* p_metadata, SPTAG::SizeType p_vectorCount, ContainSampleFunc p_containSample, SPTAG::SizeType p_dataBlockSize, bool p_checkDeleted) {
