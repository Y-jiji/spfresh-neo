...
ErrorCode Index<T>::LoadIndexData(const std::vector<std::shared_ptr<Helper::DiskIO>>& p_indexStreams) {
    ...
    for (int j = 0; j < m_options.m_iSSDNumberOfThreads; j++) {
...
#include "Core/SPANN/ParameterDefinitionList.h"
...
template <typename T>
ErrorCode Index<T>::DebugSearchDiskIndex(QueryResult& p_query, int p_subInternalResultNum, int p_internalResultNum, SearchStats* p_stats, std::set<int>* truth, std::map<int, std::set<int>>* found) const {
    if (nullptr == m_extraSearcher)
        return ErrorCode::EmptyIndex;
...
template <typename T>
int Index<T>::SelectHeadDynamicallyInternal(const std::shared_ptr<COMMON::BKTree> p_tree, int p_nodeID, const Options& p_opts, std::vector<int>& p_selected) {
...
ErrorCode Index<T>::DeleteIndex(const void* p_vectors, SizeType p_vectorNum) {
    ...
    if (m_options.m_distCalcMethod == DistCalcMethod::Cosine) {
        ...
        vectorSet.reset(new BasicVectorSet(ByteArray((std::uint8_t*)p_vectors, sizeof(T) * 1 * p_dimension, false), GetEnumValueType<T>(), p_dimension, 1));
    }
